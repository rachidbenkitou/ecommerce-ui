...
export class ProductAddEditComponent implements OnInit {
  ...
  addProduct() {
    ...
    if (this.selectedImages && this.selectedImages.length > 0) {
      this.productService.addProduct(this.productForm.value, this.selectedImages).subscribe(
        (response: any) => {
        },
  ...
  editProduct() {
    this.sppinerDeleteDisplaying = true
    this.submitButton.disabled = true
    this.productService.updateProduct(this.productForm.value?.id, this.productForm.value, this.selectedImages).subscribe(
      (response: any) => {
      },
      (error: HttpErrorResponse) => {
        this.submitButton.disabled = false
